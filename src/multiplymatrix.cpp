...
Matrix* utils::MultiplyMatrix::execute() {
    ...
    20/Sep/2019
    */
   for (int i=0; i < a->getNumRows(); i++) { // for each row in a
       for (int j=0; j < b->getNumCols(); j++) { // for each column in b
           double c_ij = 0; // placeholder for entry ij of our output matrix c
           for (int k=0; k < a->getNumCols(); k++) { // for each entry in the current row of a
